...
void StereoNode::Acquire() {
  while (is_acquire() && ros::ok()) {
    ...
    if (left_ros_.RequestSingle()){
      ...
      // ROS_INFO("%lf,  %lf", expose_duration.toSec(), right_expose_duration.toSec());
      // ROS_INFO("%.10f,  %.10f", expose_duration.toSec(), right_expose_duration.toSec());
      ros::Time time_now = ros::Time::now();
      ros::Time time = time_now + expose_duration;
      left_ros_.PublishCamera(time);
      ros::Time time2 = time_now + right_expose_duration;// + right_expose_duration;
...
void StereoNode::Setup(Flea3DynConfig& config) {
  ...
  right_ros_.camera().Configure(config_cpy);//left_ros_.camera().Configure(config_cpy);
  ...
  config_cpy.trigger_source = 2;
  config_cpy.strobe_control = -1;
